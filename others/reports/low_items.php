...
$query = "SELECT
        CONCAT_WS(' - ',item_name,item_code) AS item_name,  
        ims_main_stock_summary.item_qty, 
        ims_item_details.reorder_level, 
        DATE_FORMAT(item_last_update_date_time,'%Y-%m-%d') AS last_Update_Date
        FROM
        ims_item_details
        INNER JOIN
        ...
        WHERE
        ims_item_details.reorder_level >= `item_qty`";
...
<table class="buttonSection">
    <tr>
        <td>
            <button type="button" onclick="window.location='./?stock'" class="update-btn">Back</button>
        </td>
        <td>
