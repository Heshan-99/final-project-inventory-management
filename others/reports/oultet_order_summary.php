...
if (isset($_GET['all'])) {
    $query = "SELECT
            ...
            ims_outlet_bill_summary.outlet_bill_date_time, 
    ...
    $query = "SELECT
            ...
            ims_outlet_bill_summary
            WHERE
            DATE_FORMAT(outlet_bill_date_time, '%Y-%m-%d') BETWEEN '{$_GET['from']}' AND '{$_GET['to']}'";
}
    ...
    <body>
        ...
        <table style="margin-top: -40px;">
            ...
            <tbody>
                ...
                foreach ($data as $x) {
                    echo '<tr>
                                <td>' . $x['outlet_bill_no'] . '</td>
                                <td>' . $x['outlet_bill_date_time'] . '</td>
                                <td>' . $x['outlet_bill_amt'] . '</td>
                                <td>' . $x['paid_amt'] . '</td>
                                <td>' . $x['balance_amt'] . '</td>
